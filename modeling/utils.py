...
# All Rights Reserved.
"""
SHAPE_NET = os.path.join("./pretrained_models/", "distance_measures_regressor.pth")
    ...
    def forward(self, inputs, targets, do_rmi=None):
        ...
        for i in range(0, inputs.shape[0]):
            if not self.batch_weights:
                ...
                self.nll_loss.weight = weights
            # print(F.log_softmax(inputs[i].unsqueeze(0), dim=1).shape)
            # print(targets[i].unsqueeze(0).shape)
            loss += self.nll_loss(F.log_softmax(inputs[i].unsqueeze(0), dim=1),
                                  targets[i].unsqueeze(0),)
        return loss

def structure_loss(pred, mask, iou=True):
...
class JointEdgeSegLoss(nn.Module):
    ...
    def edge_attention(self, input, target, edge):
        ...
        # print((edge.max(1)[0] > 0.8).shape)
        # print(target.shape)
        return self.seg_loss(input, 
                             torch.where((edge.max(1)[0] > 0.8).unsqueeze(1), target, filler))
